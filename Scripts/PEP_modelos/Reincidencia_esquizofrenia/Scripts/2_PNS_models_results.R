...
rec <- extract_recipe(Model_fit)


prep(rec, training(PNS_split) )

juice(rec,training(PNS_split))
X <- prep(rec, training(PNS_split)) %>% 
  juice() %>% 
  select(-PNS_reincidencia) %>% 
  as.matrix()


predict(Model_fit, training(PNS_split), type = "prob") %>% 
  rownames_to_column("rowid") %>% 
  # filter(.pred_No == min(.pred_No) | .pred_No == max(.pred_No)) %>% 
  gt() %>% 
